...
public class AdminDAO {
    ...
    private String getCondition(String filter) {
        if ("Tuần này".equals(filter)) return "DATEDIFF(wk, created_at, GETDATE()) = 0";
        if ("Tháng này".equals(filter)) return "YEAR(created_at) = YEAR(GETDATE()) AND MONTH(created_at) = MONTH(GETDATE())";
        if ("Năm nay".equals(filter)) return "YEAR(created_at) = YEAR(GETDATE())";
        return "CAST(created_at AS DATE) = CAST(GETDATE() AS DATE)"; // Mặc định là "Hôm nay"
    }
    ...
    // --- 2. TOP SẢN PHẨM THEO BỘ LỌC (ĐÃ CHUYỂN LIMIT THÀNH TOP 5) ---
    public Map<String, Integer> getTopSellingProducts(String filter) {
        Map<String, Integer> topProducts = new HashMap<>();
        String sql = "SELECT TOP 5 p.product_name, SUM(od.quantity) as total_qty FROM order_details od " +
                "JOIN products p ON od.product_id = p.product_id " +
                "JOIN orders o ON od.order_id = o.order_id " +
                "WHERE " + getCondition(filter).replace("created_at", "o.created_at") + " " +
    ...
    public Map<String, Double> getChartData(String filter) {
        ...
        if ("Năm nay".equals(filter)) {
            ...
            String sql = "SELECT MONTH(created_at) as label, SUM(total_amount) as total FROM orders WHERE YEAR(created_at) = YEAR(GETDATE()) GROUP BY MONTH(created_at)";
            ...
            String sql = "SELECT CAST(created_at AS DATE) as order_date, SUM(total_amount) as total FROM orders WHERE created_at >= DATEADD(day, -6, CAST(GETDATE() AS DATE)) GROUP BY CAST(created_at AS DATE)";
            try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql); ResultSet rs = pstmt.executeQuery()) {
                while(rs.next()) {
    ...
    // --- API TỔNG QUAN TOÀN BỘ CỬA HÀNG (GLOBAL ANALYTICS) ---
    public String getGlobalAnalytics(String filter) {
        JsonObject root = new JsonObject();
        String timeCond = "1=1";

        if ("Hôm nay".equals(filter)) timeCond = "CAST(created_at AS DATE) = CAST(GETDATE() AS DATE)";
        else if ("Tuần này".equals(filter)) timeCond = "DATEDIFF(wk, created_at, GETDATE()) = 0";
        else if ("Tháng này".equals(filter)) timeCond = "YEAR(created_at) = YEAR(GETDATE()) AND MONTH(created_at) = MONTH(GETDATE())";
        else if ("Năm nay".equals(filter)) timeCond = "YEAR(created_at) = YEAR(GETDATE())";

        try (Connection conn = DatabaseConnection.getConnection()) {

            // 1. Doanh thu & Tổng đơn
            String sqlKpi = "SELECT COUNT(order_id) as TotalOrders, ISNULL(SUM(total_amount), 0) as TotalRevenue FROM orders WHERE " + timeCond;
            ...
            // 2. Top 5 Nhân viên
            String sqlTopEmp = "SELECT TOP 5 u.full_name, COUNT(o.order_id) as OrderCount, SUM(o.total_amount) as Revenue " +
            ...
            // 3. Top 5 Sản phẩm
            String sqlTopProd = "SELECT TOP 5 p.product_name, SUM(od.quantity) as TotalSold, SUM(od.quantity * od.unit_price) as ProdRev " +
                    ...
                    " GROUP BY p.product_id, p.product_name ORDER BY TotalSold DESC";
